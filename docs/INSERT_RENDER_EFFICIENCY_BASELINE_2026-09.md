...
## Actual execution target — 12 September follow-up

Current `src/lib/pipelineInvocationSnapshot.ts` limits remote blocks to `timeline_assemble`, `documotion_short`, `novita_render_images`, and `novita_render_video`. The actual `visual_inserts`/`quote_overlays` loops therefore render on `runPipelineTask` (`src/trigger/runPipeline.ts`, `machine: "large-1x"`), not the heavier `render-block` child or an offloaded GPU. Trigger's [current machine documentation](https://trigger.dev/docs/machines) describes large-1x as four vCPUs/eight GB and large-2x as eight vCPUs/16 GB. Task-time overrides and actual cgroup CPU visibility would still need observation before attributing local thread behavior or timings to cloud billing.

The shared Remotion bundle cache was also inspected as a possible duplicate-work target. It stores only the completed URL, so overlapping cold calls could start duplicate bundles; however, the actual insert and quote loops inspected here are sequential. No speculative promise-cache patch or measured production saving is claimed for that hypothesis.

The [90-frame native thread comparison](INSERT_ENCODER_THREAD_COMPARISON_2026-09.md) completed with the unchanged Remotion stitcher and preserved browser PNGs. Both caps fail exact decoded parity on 83/90 frames; an unchanged-auto repeat preserves all pixels/alpha/timestamps, so the differences are not blamed on observed baseline nondeterminism. Auto wall time varied 50.20→39.60 seconds; four threads took 23.96 and two took 29.02 seconds. Four threads is slightly less faithful to the actual PNG source; two is more faithful on this fixture, with visible auto/four-thread alpha blocks/streaks absent from the source. That supports a separate full-length source-quality study, not adoption or a changed exact-parity verdict. The study must preserve the actual silent-Opus/mux path and cannot reuse an unbound historical browser pass as its baseline. No application renderer, codec, quality setting, stage routing or machine tier has been changed.
