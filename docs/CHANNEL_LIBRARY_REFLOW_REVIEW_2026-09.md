...
(zero errors, 33 existing warnings). The overlay results alone do not establish
deployment; the separate exact-production verification below now does.
...
## Exact production verification — 10 September 2026

Released in `98ca6829bfebff84dcc8a2b95abe1bbfccbbe33e`. GitHub CI
`34418738320` succeeded, including the production-readiness suite and both cloud
deployments. Convex reported ready at 00:03:39 UTC. Trigger completed at
00:05:41 UTC with version `20260910.1`, worker
`worker_cmturn8q0ihrv0ineo92kwyr4`, content hash
`889ba660b5fca12c3315d4fbdad96152`. These are deployment receipts, not proof of
an actual new video run.

Vercel `dpl_8zRwgPt1fGMdSZtYANNNAa1cPhjJ` is READY for the exact revision and
canonical `youtube-studio-ai.vercel.app` production alias. Health after the
cloud deployment reports that revision. Provider/log receipts are retained in
`/tmp/ysa-98ca682-production-DCjBQN`.

The final frozen browser proof passes **five of five** exact-production
profiles without a stylesheet overlay (`css: null`, terminal exit 0):
`/tmp/ysa-channel-library-proof-Sca9wg/results.json`. The main Library's three
enlarged-text profiles also complete without failures at
`/tmp/ysa-library-truth-production-gjBKOf/run-RrSJFB/results.json`.
All eight actual native master previews reach 15 seconds with duration
200.551 seconds, readyState 4, no media error and successful HTTP 206 ranges.
Navigation, native controls, focus restoration, metadata, sorting and bounds
checks pass. The reviewer inspected all eight native screenshots and the
layout captures; root independently inspected channel header/card captures
and the enlarged-phone main Library player. This is playback/layout proof,
not approval of the legacy video's art, narration or publishing readiness.

Full scope and evidence limitations remain in
`/tmp/ysa-98ca682-production-DCjBQN/UI_REVIEW.md`. No production records,
thumbnail assignments, credentials or media were changed by verification.
