...
**Current-state update, 10 September:** the compile-seed divergence described
below is historical and has since been repaired and deployed as `d24a00e`.
The [shared seed repair](CHANNEL_PROGRAM_ROUTE_SEED_REPAIR_REVIEW_2026-09.md)
projects only the actually admitted route at all three real callers, after
frozen/private/weekly seed selection and before credential bootstrap. Its
33-case actual-caller regression is part of the current production suite.
Do not implement that repair again or add an arithmetic payload allowlist.
The missing arithmetic intent/request producer, policy-compatible route,
actual speech/visual qualification and automatic channel integration remain
open. The retained before audit below still explains the original finding.
