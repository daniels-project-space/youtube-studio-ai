...
**Web release verified:** documentation-inclusive revision `905797bdf8ae45729b3b3a1a81378e525752cce5` is pushed to main and the checkpoint branch. Vercel's provider API confirms `dpl_HhREW36pk43rP9rZk5t5hG9EAKnk` READY / production, bound to the canonical `youtube-studio-ai.vercel.app` alias; `/api/health` returns the same SHA. The GitHub Vercel status also points at a separate checkpoint preview (`dpl_8GKqpwYM2HsH9ovrbKu7qPVisyhm`), which must not be mistaken for production. Cloud CI `34690403868` is confirmed in progress at job `103544520854` (typecheck/full quality gate); Convex/Trigger deployment is not yet claimed. Recheck that exact handle rather than rerunning it on an observation timeout.
