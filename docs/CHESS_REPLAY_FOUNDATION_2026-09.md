...
The current bridge requires one legal ply per scene and exact one-sentence-per-ply cue timing. One-move lessons, richer reviewed analysis layered around legal-move sentences, translations and several plies inside a broader Story Spine beat remain subsequent integration work. Nothing is advertised in the catalog yet.
...
3. Connect the new cue binder to actual normalized TTS/Story Spine receipts, supporting richer reviewed analysis around a legal ply and legitimate pauses/chapters. The current fixture timing is deliberately synthetic and cannot substitute for a TTS receipt in production.
