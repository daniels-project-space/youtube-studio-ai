...
**12 September exact-source follow-up:** pinned `deploy/archiveContext.js` uses its default exclusions plus the repository `.gitignore`; it does not read `.vercelignore` or a `.triggerignore`. `graphify-out/` is correctly excluded by `.gitignore`, but retained test media is not. The successful `9cbab32` native deployment reports a 124.47 MB uploaded context. This is an actual transfer-size observation, not the final worker-image size or billing. Native `handleNativeBuildServerDeploy` archives the workspace and initializes with `contentHash: "-"`; the later JavaScript hash still must not stand in for Python/additional-file identity. A narrower context is a concrete future optimization, but adding a nonfunctional `.triggerignore`, hiding required renderer assets, or globally ignoring future test evidence is not a valid implementation. No deployment script or ignore policy was changed.
...
## 20 September: Native context reduction implemented, not deployed

The CI Trigger step now excludes only root `test-fixtures/` during its native
upload via a temporary, restored `.gitignore` addition. Evidence remains tracked
and available to the full quality gate; manual deployments are unchanged. An
offline comparison using the pinned CLI's actual archive function measures a
33.06% smaller compressed context, with every non-fixture entry preserved except
the temporary ignore policy. See batch 19 in
[the usage audit](infrastructure-usage-audit-20260920.md) for exact byte counts,
actual-shell regression and reproducible verification. This is not a deployment
skip gate, live cloud-build qualification or measured billing reduction.
