...
**Platform check, 10 September:** YouTube's current upload guidance admits
square/vertical videos up to three minutes as Shorts. Shorts over one minute
with an active copyright claim are blocked globally, so usable visuals alone
do not establish a publishable format. These are platform constraints, not a
claim that this app can currently inspect all claims through its API.
[Official format guidance](https://support.google.com/youtube/answer/15424877?hl=en),
[computer upload requirements](https://support.google.com/youtube/answer/12779649?co=GENIE.Platform%3DDesktop&hl=en).

Engineering decision: retain native 9:16 as the deliberate initial profile,
not a claim that YouTube requires only that aspect. Give the new short format
its own duration contract instead of inheriting the existing 300-second
default. Validate actual encoded geometry/duration after intro/outro, not only
the requested scene duration. Include an exact boundary and one-frame-over
negative; never trim a lesson's last answer to make it fit. Retain existing
asset-rights and private-release checks; no automatic claim dispute or
claim-free attestation is introduced. This is a qualification requirement,
not an enabled Shorts route or a weakened music policy.
...
automatic producer and the historical creator/runtime seed-key mismatch. The
latter is already resolved by the released [shared seed repair](CHANNEL_PROGRAM_ROUTE_SEED_REPAIR_REVIEW_2026-09.md);
do not duplicate it while adding the still-missing intent-bound producer.
The [critical
...
**Shared QA efficiency/correctness work:** the baseline fresh visual review
ran before transcript checks; failing-source fixtures invoked a reviewer.
The reviewed real-caller preflight now moves the existing source checks earlier,
with unchanged final-master identity and no duplicate accepted-run ASR pass.
It distinguishes proven content defects from source-read/ASR availability.
A separate clean ordinary-narration port passed all 651 direct readiness tests,
twelve unchanged audits and the actual combined production build. Exact
`c49039c` is deployed with successful CI `34422175588`, exact Vercel alias,
Convex and Trigger `20260910.2`. Six independent no-overlay browser profiles
pass, including all fourteen real channel options in each. This
improvement does not depend on unfinished arithmetic admission.
Fixture invocation savings are not an invoice or elapsed-time saving claim.

The [transcript count/coverage repair](NARRATION_TRANSCRIPT_COUNT_COVERAGE_REVIEW_2026-09.md)
is now deployed in exact 98ca on Vercel, Convex and Trigger 20260910.1. The real
Python-to-TypeScript 22-case bridge and complete 650-test readiness suite passed.
The producer counts timestamp entries; both full-receipt boundaries verify
complete ordered lexical coverage. Existing WER/recall policy, model pins,
semantic word-versus-digit limits and historical summary-only contracts remain
unchanged. This is not independent critical-number speech validation.

**Arithmetic visual connector, active held implementation:** independently
replayed typed calculations must reach Scene Manifest as a versioned lesson
envelope and ordered node/phase references, not generic circles or equation
text parsed from labels. Rebind the complete current request, preparation,
script and finalized audio clock. Use existing measured sentence ends for a
conservative result reveal; current/future answer-role tokens must not exist
before their reveal frame. Never guess timing or require downstream final QA
as an upstream render input. Unmarked graph output stays unchanged. The first
slice explicitly refuses unsupported chapter timing/portrait layouts instead
of implying those variants work. Pure synthetic-clock contract tests are
separate from actual measured speech, renderer pixels and full-channel proof.
The detailed source audit and before caller are retained at
`/tmp/ysa-arithmetic-renderer-audit-h2X0my/PLAN.md` and `before.json`.

**Real voice boundary findings, 10 September:** the read-only audition-path
audit at `/tmp/ysa-worked-example-audio-path-Mo7rJF/PLAN.md` confirms actual
request→preparation→script→independent critic→TTS seams, but no admitted
automatic arithmetic request producer. The canonical draft has not been
accepted by the real critic. Its fit to the existing POV/re-hook rubric must be
tested; never hand-author a passing approval or rewrite output outside the
module. Provider key/model metadata is not a generation or credit receipt.

An independent execution of the real TTS splitter on the generated three-step
draft returns **five** speech units. The first visual candidate incorrectly
expected eight, separating each step announcement from its lowercase
calculation. Its 67 synthetic-clock cases therefore did not prove the actual
connection. Root's failing before oracle is retained at
`/tmp/ysa-arithmetic-next-checkpoint-ghJ7BN/actual-tts-unit-boundary-before.log`.
The corrected [visual plan](WORKED_EXAMPLE_VISUAL_PLAN_REVIEW_2026-09.md) now
matches those actual five units. Root independently reran all 68 contract cases,
the actual splitter, full typecheck and the held caller regressions in composed
checkpoint `df6cc81`; that checkpoint is saved separately, not on production.
The rejected eight-unit sources and failing before oracle remain retained.
No voice-module change or extra take was used to satisfy the mistaken shape.

Separately, TTS currently tolerates up to two estimated segment durations and
can reconcile them against the measured final duration without persisting
per-segment measurement provenance. A bound timing hash is not evidence that
each end was directly measured. Carry existing measurement provenance for
qualification, and recover local probes using retained paid bytes. Do not
buy speech again or treat the generic cue tolerance as a never-early reveal
guarantee. Exact-byte/timing, independent critical speech and actual rendered
before/at/after reveal checks remain required before admission.

**Next bounded implementation decisions:** preserve segment-clock provenance as
an optional strict child of the existing audio binding, not a new global
timing service. Old paid audio remains restorable; only the new arithmetic
visual admission requires all-measured evidence. Local probe recovery may reuse
the exact segment bytes but cannot synthesize replacements. Separately, a
dedicated marked branch in the existing Scene Compiler will present typed
operands, operators, safe step labels and only eligible result-role tokens.
Use existing theme primitives and no generated images; verify actual native
frames, generated dense cases, negative visual oracles and unchanged ordinary
scene pixels. These are active implementation scopes, not qualified output.

**Implementation review:** those two bounded slices now pass independently in
isolated worktrees; the renderer has real native output and defective-renderer
rejection, while the clock records actual local measurements and survives
final-only cache restoration. Ordinary rendering remains pixel-identical and
all six ordinary TTS provider/mode transports retain exact bytes, timing and
cost. Composition is still required. The 26.618776s actual final tone versus
26.776144s sum-of-part-format-durations counterexample demonstrates why a
container measurement alone is not an assembled speech clock. Diagnose sample
and packet timing at the actual concat/encode seam before changing construction;
do not convert a known discrepancy into an arbitrary acceptance margin.

**Automatic route constraint to resolve:** the existing illustrated-explainer
family advertises a minimum 60s, whereas a short generated worked example can
be materially shorter. A future explicit lesson intent/route must describe
honest duration and content scope. Do not append a second script producer,
claim deterministic arithmetic is researched topical evidence, pad narration
to satisfy generic runtime promises, or weaken an unrelated family's contract.
