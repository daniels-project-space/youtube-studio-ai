...
- **Title baseline:** [current title-module audit](TITLE_MODULE_BASELINE_2026-09.md) preserves the real call chain, reproduced false-pass and contradictory fallbacks. [Eight real source packets are now frozen](TITLE_CORPUS_SOURCE_AVAILABILITY_2026-09.md), including five full narrations, with file/source/narration hashes. They are source-backed regressions, not exact historical invocation replays: missing context remains explicit. No live baseline generation or measured output improvement has been claimed.
...
- [ ] **80 — Audit reactive UI queries.** Remove duplicate subscriptions and frequently changing arguments that defeat cache reuse; keep one source of truth per visible datum.
  ...
  **Measured next case (9 September):** the run-detail thumbnail repair uses the proven `videos.getVideoDetail` resolver, but its consumer only needs title, thumbnail key/presentation and video key. A read-only five-run audit found 5–7 indexed queries per evaluation, 34,583 combined serialized characters of unused script/SEO response fields, and 16 duplicate source-asset row reads across the sample. These are logical query/row and serialized-character counts, not measured invoices or wire bytes. Follow up with one narrow media-and-current-thumbnail query that reuses the exact existing provenance/LoFi/master selection rules, removes the duplicate asset subscription and avoids script probes. Do not trade away selector correctness or interpret this modest sample as a fleet-wide cost estimate.
