...
async function main(): Promise<void> {
  const output = process.env.OUT_PATH ?? path.join(
    ...
    "output",
    "documotion",
    "voyager-golden-record-story-30-v5-visual.mp4",
  );
  ...
  const composition = await selectComposition({ serveUrl, id: "VoyagerGoldenRecordStory30V5", inputProps: {} });
  await renderMedia({
    serveUrl,
    composition,
    inputProps: {},
    ...
    onProgress: ({ progress }) => {
      ...
      if (percent % 10 === 0) console.log(`voyager-story-30-v5 render ${percent}%`);
    },
  });
  console.log(output);
}
