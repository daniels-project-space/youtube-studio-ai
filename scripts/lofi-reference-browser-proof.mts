...
const outputDir = await mkdtemp(join(tmpdir(), "ysa-lofi-reference-"));
...
try {
  for (const [name, width, fontSize] of [["desktop",1440,16],["phone",390,16],["small-phone",320,16],["desktop-large",1440,32],["phone-large",390,32]] as const) {
    ...
    try {
      ...
      await page.goto(base + "/lofi", { waitUntil: "domcontentloaded" });
      ...
      if (await catalog.count() !== 1) failures.push(`${name}: catalog return link is missing`);
        ...
        const card = page.locator('details[data-module-key="lofi"]');
        ...
        await card.getByRole("link", { name: "Open Lo-fi references", exact: true }).click();
        await page.waitForURL(base + "/lofi"); await title.waitFor();
      }
