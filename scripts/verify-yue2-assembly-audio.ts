...
const [reference, master] = process.argv.slice(2);
assert.ok(reference && master, "Usage: verify-yue2-assembly-audio.ts LOOP.wav MASTER.mp4");
...
const readWindow = (path: string, sample: number) => execFileSync(ffmpeg, ["-v", "error", "-ss",
  (sample / 48000).toFixed(9), "-i", path, "-t", "1", "-map", "0:a:0", "-c:a", "pcm_f32le", "-f", "f32le", "pipe:1"],
...
for (const repeat of new Set([0, Math.floor(Math.max(0, lastRepeat) / 2), Math.max(0, lastRepeat)])) {
  const frame = referenceFrame + repeat * loopFrames;
  ...
  results.push({ repeat, sample: frame, seconds: frame / 48000, correlation, rmsRatio });
}
console.log(JSON.stringify({ version: "yue2-assembly-audio-alignment/v1", reference, master, durationSec: duration,
