...
assert.match(detail, /<NicheMotionGlyph niche=\{id\.niche\} channelName=\{channel\.name\}/);
assert.match(detail, /className=\{styles\.heroMotif\}/);
...
assert.match(wizard, /Private proof · live/);
assert.match(wizard, /Render in progress/);
assert.match(wizard, /proofOrbit/);
...
assert.match(channels, /channelCategoryLabelFor\(c\)/);
