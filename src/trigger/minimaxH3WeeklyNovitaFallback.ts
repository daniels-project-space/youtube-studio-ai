/**
 * Terminal provider fallback for a weekly H3 order held by Salad capacity.
 *
 * The capacity waiter owns Salad's medium/high retry window. Once that
 * bounded day has elapsed, this task renders the exact frozen packet through
 * Novita. Each completed shot is claimed before the aggregate receipt, so a
...
async function readAggregate(key: string, ownerId: string): Promise<PersistedWeeklyReceipt | null> {
  ...
  try { bytes = await getObjectBytes(key); } catch (error) { if (notFound(error)) return null; throw error; }
  let parsed: unknown;
...
async function readFallbackClaim(args: { receiptKey: string; orderKey: string; requestKey: string; outputKey: string }): Promise<MiniMaxH3RenderedVideo | null> {
  const key = miniMaxH3WeeklyJobReceiptKey(args.receiptKey, args.requestKey);
  let bytes: Uint8Array;
  try { bytes = await getObjectBytes(key); } catch (error) { if (notFound(error)) return null; throw error; }
  let parsed: PersistedWeeklyJobReceipt;
  try { parsed = JSON.parse(new TextDecoder().decode(bytes)) as PersistedWeeklyJobReceipt; } catch { throw new Error("weekly H3 Novita fallback job receipt is not valid JSON"); }
...
async function persistFallbackClaim(args: { receiptKey: string; orderKey: string; result: MiniMaxH3RenderedVideo }): Promise<void> {
  ...
  try { await putObject(key, body, { contentType: "application/json", metadata: { "h3-job-receipt": "minimax-h3-weekly-job/v1", sha256: sha256Hex(body) }, ifNoneMatch: "*" }); }
  catch (error) { const status = (error as { $metadata?: { httpStatusCode?: number } }).$metadata?.httpStatusCode; if (status !== 409 && status !== 412) throw error; }
  const persisted = await readFallbackClaim({ receiptKey: args.receiptKey, orderKey: args.orderKey, requestKey: args.result.requestKey, outputKey: args.result.receipt.output.r2Key });
  if (!persisted || canonicalJson(persisted.receipt) !== canonicalJson(args.result.receipt)) throw new Error("weekly H3 Novita fallback job receipt changed after create-only write");
}
...
async function readFrozenPacket(payload: MiniMaxH3WeeklyBatchArgs): Promise<string[]> {
  ...
  if (packet.schema !== "minimax-h3-weekly-request/v1" || packet.orderKey !== payload.orderKey || canonicalJson(packet.requestKeys) !== canonicalJson(sourceRequestKeys) || canonicalJson(packet.jobs) !== canonicalJson(payload.jobs)) {
...
export const minimaxH3WeeklyNovitaFallbackTask = task({
  ...
  run: async (rawPayload: MiniMaxH3WeeklyBatchArgs) => {
    ...
    if (prior) {
      ...
      const preparedFootageKey = payload.preparedFootage ? await materializePreparedFootage(payload.preparedFootage, await readPreparedFootageManifest(payload.preparedFootage), payload.jobs, renderedResultsFromPersistedReceipt(prior), priorProvider === "salad" ? { provider: "salad", execution: "weekly-batch" } : { provider: priorProvider, execution: "weekly-fallback" }) : undefined;
    ...
    await Promise.all(Array.from({ length: Math.min(8, payload.jobs.length) }, async () => {
      for (;;) { const index = next++; if (index >= payload.jobs.length) return; results[index] = await readFallbackClaim({ receiptKey: payload.receiptKey, orderKey: payload.orderKey, requestKey: requestKeys[index]!, outputKey: payload.jobs[index]!.output.r2Key }) ?? undefined; }
    }));
    ...
    try { await putObject(payload.receiptKey, body, { contentType: "application/json", metadata: { "h3-batch-receipt": "v2", "h3-batch-sha256": sha256Hex(body), "h3-fallback-provider": "novita" }, ifNoneMatch: "*" }); }
    catch (error) { const status = (error as { $metadata?: { httpStatusCode?: number } }).$metadata?.httpStatusCode; if (status !== 409 && status !== 412) throw error; const winner = await readAggregate(payload.receiptKey, payload.ownerId); if (!winner) throw error; return { state: "reconciled" as const, provider: "novita" as const, receiptKey: payload.receiptKey }; }
    const preparedFootageKey = payload.preparedFootage ? await materializePreparedFootage(payload.preparedFootage, await readPreparedFootageManifest(payload.preparedFootage), payload.jobs, results, { provider: "novita", execution: "weekly-fallback" }) : undefined;
