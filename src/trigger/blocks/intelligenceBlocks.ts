...
/**
 * Package-only finishing pass: preserve the admitted title exactly,
 * merge/screen tags against identity.bannedWords, append
...
export const metadataOptimized: Block = {
  ...
  run: async (ctx) => {
    ...
    if (niche) {
      try {
        const estimate = await convex().query(api.seo.viewEstimate, { ownerId: ctx.ownerId, niche, tags });
        estimatedViews = estimate.estimatedViews;
        estimatedViewsSource = estimate.source;
      } catch (error) { ctx.log("metadata: viewEstimate unavailable, retaining niche estimate: " + String(error)); }
    }
    ctx.log("metadata: admitted [" + m.frame + "] " + m.clickScore + "/10 — " + title);
    return {
      title, description, tags, pinnedComment: m.pinnedComment, titleAlternate: m.titleAlternate,
      titleDecision: m.titleDecision,
      thumbnailDescription: buildThumbnailDescription({ title, topic, scriptExcerpt,
        ...(serializedEpisodePrompt ? { serializedEpisodeContext: serializedEpisodePrompt } : {}) }),
      estimatedViews, estimatedViewsSource, [COST_PATCH_KEY]: costUsd,
    };
  },
};
