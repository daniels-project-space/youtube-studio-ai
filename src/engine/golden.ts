...
export const GOLDEN_SPINE: GoldenStage[] = [
  ...
  { stage: "sound", blocks: ["music_program_plan", "music_arrangement_plan", "music"], note: "A route-sealed original-music program locks one episode’s instrumental and loop-visual intent before the channel-scoped score or long-form music product is generated. The opt-in music_arrangement_plan separately seals composer-authored intent for evaluation only; it does not activate a production route or qualify generated audio." },
...
export const GOLDEN_MODULES: GoldenModule[] = [
  ...
  {
    key: "chess-replay-private-review",
    stage: "brief",
    title: "Chess Replay — Registered Source-Bound Private Review",
    engine: "supplied PGN/source packet → legal replay → exact move transcript → shared TTS timing receipt → deterministic board manifest",
    how: "A bounded legal-mainline adapter for controlled review. It reconstructs only the supplied game, gives each legal ply one exact spoken sentence, and refuses altered scripts or timing before the existing TTS/Scene Compiler handoff. It does not discover games, prove a source authentic or licensed, add analysis, or qualify final voice/video quality.",
    gates: [
      "explicit source packet: locator, rights note, expected ply count and final FEN",
      "source-only script integrity before the existing paid narration block",
      "one measured TTS cue per legal ply; reordered, rewritten, overlapping or shortened cues reject",
      "native board graph uses the exact replay, narration-plan and timing fingerprints",
      "private review only: no channel-creator route, Golden promotion, auto-discovery, provider qualification or publishing",
    ],
    status: "registered",
  },
  {
