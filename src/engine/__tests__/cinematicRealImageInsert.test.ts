...
/**
 * End-to-end behavioral test for the `realImageInsertQuery` field on
...
function coverageShot(args: {
  ...
  return {
    ...
    ...(args.realImageInsertQuery ? { realImageInsertQuery: args.realImageInsertQuery } : {}),
  };
}
  ...
  const input: CinematicCaseSequenceInput = {
    ...
    beats: [
      ...
      },
      {
        ...
        shots: [
          // The narrow real-image-insert exception: a cited source_proof
          // evidence_insert extreme close-up with no mannequin cast.
          coverageShot({ id: "cinematic-shot-verdict-proof", t0: 12, t1: 16, purpose: "evidence_insert", mode: "source_proof", scale: "extreme_close", move: "truck_right", cut: "reveal", tension: "reversal", realImageInsertQuery: "Regional Court Archive verdict document official letterhead" }),
...
function main() {
  ...
  // ---- POSITIVE: a well-formed real-image-insert shot is admitted --------
  const report = evaluateCinematicCaseSequence(args);
  assert.equal(report.safe, true, `well-formed real-image-insert shot must be admitted: ${JSON.stringify(report.issues.slice(0, 5))}`);
  ...
  assert.ok(proofScene, "the real-image-insert shot must produce a generated scene");
  assert.ok(mapScene, "the sibling non-real-image shot must produce a generated scene");

  // The reviewed search query passes through verbatim onto the generated
  // scene so a render pipeline can resolve the actual photograph.
  assert.equal(proofScene!.realImageInsertQuery, "Regional Court Archive verdict document official letterhead", "realImageInsertQuery must pass through to the generated scene");
  assert.equal(mapScene!.realImageInsertQuery, undefined, "an unrelated shot must not carry a real-image-insert query");
  ...
  assert.equal(wrongPurposeReport.safe, false, "a real-image-insert shot with the wrong coveragePurpose must be rejected");
  assert.ok(
    wrongPurposeReport.issues.some((entry) => entry.code === "real_image_insert_invalid" && entry.message.includes("not an evidence_insert shot")),
  ...
  assert.equal(wrongModeReport.safe, false, "a real-image-insert shot with the wrong visualMode must be rejected");
  assert.ok(
    wrongModeReport.issues.some((entry) => entry.code === "real_image_insert_invalid" && entry.message.includes("does not use source_proof")),
  ...
  assert.equal(wrongScaleReport.safe, false, "a real-image-insert shot not framed extreme_close must be rejected");
  assert.ok(
    wrongScaleReport.issues.some((entry) => entry.code === "real_image_insert_invalid" && entry.message.includes("not framed extreme_close")),
  ...
  assert.equal(withCastReport.safe, false, "a real-image-insert shot with a mannequin cast must be rejected");
  assert.ok(
    withCastReport.issues.some((entry) => entry.code === "real_image_insert_invalid" && entry.message.includes("carries a mannequin cast")),
    `expected the 'carries a mannequin cast' issue, got: ${JSON.stringify(withCastReport.issues.map((entry) => entry.code))}`,
  );

  // ---- REGRESSION: an ordinary shot with no realImageInsertQuery is unaffected
  const noQuery = structuredClone(input);
  delete (noQuery.beats[1]!.shots[0] as { realImageInsertQuery?: string }).realImageInsertQuery;
  ...
  noQuery.editorialReview.reviewedSequenceFingerprint = cinematicCaseSequenceContentFingerprint(noQuery);
  const noQueryReport = evaluateCinematicCaseSequence({ ...args, input: noQuery });
  assert.equal(noQueryReport.safe, true, `omitting realImageInsertQuery entirely must keep validating unchanged: ${JSON.stringify(noQueryReport.issues.slice(0, 5))}`);

  console.log("cinematic real-image-insert exception behavioral test passed");
}
