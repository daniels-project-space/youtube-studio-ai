...
export const YuE2AuditionSubmissionSchema = z.object({
  ...
  verdict: z.enum(["needs_work", "rejected", "promising"]),
...
export type YuE2AuditionRecord = YuE2AuditionSubmission & { reviewedAt: number; reviewerId: string; productionApproved: false };
...
export function validateYuE2Audition(value: unknown, evidence: {
  ...
  if (submission.verdict === "promising" && (evidence.technicallyBlocked || !evidence.contextRetained ||
    ...
    throw new Error("Promising requires complete listening, retained context, clear technical checks and passing documented judgments");
  }
  return submission;
}
