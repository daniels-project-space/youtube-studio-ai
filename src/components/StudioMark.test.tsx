...
assert.match(decorative, /data-studio-mark="signal-frame"/);
assert.match(decorative, /aria-hidden="true"/);
assert.equal((decorative.match(/data-node="orbit"/g) ?? []).length, 1);
assert.equal((decorative.match(/data-node="source"/g) ?? []).length, 1);
...
console.log("Studio signal mark contracts passed");
