...
assert.match(globalCss, /\.channel-card-grid[\s\S]*min\(100%, 280px\)/);
...
assert.doesNotMatch(globalCss, /JARVIS|jarvis/i,
  "the Studio must not ship a floating assistant over production controls");
...
assert.match(livePipeline, /aria-pressed=\{inspectionPhase === summary\.phase\}/);
