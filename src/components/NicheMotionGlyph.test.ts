...
assert.equal(channelMotionMotifFor({ niche: "Children's Learning", channelName: "Little Lantern" }), "storybook");
...
assert.match(css, /data-motif="storybook"/);
assert.match(css, /@keyframes bookTurnLeft/);
assert.match(css, /@keyframes storybookSpark/);
