...
function Review({ runId, reload }: { runId: string; reload: () => void }) {
  ...
  return <section aria-label="YuE candidate review" className={styles.review}>
    ...
    {!quality.durationMatches && <p className={styles.warning}>{quality.naturalLoopDurationAccepted
      ? "Natural-length loop source. Exact video duration remains an assembly check."
      : quality.nativeDurationMatches
