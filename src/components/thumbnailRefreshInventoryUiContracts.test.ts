...
assert.match(panel, /priority = false/);
assert.match(panel, /loading=\{priority \? "eager" : "lazy"\}/);
assert.match(panel, /fetchPriority=\{priority \? "high" : "auto"\}/);
assert.match(panel, /priority=\{index < 4\}/);
assert.doesNotMatch(panel, /i\.ytimg\.com/);
assert.match(panel, /stale\/dead public YouTube artwork/);
...
assert.match(panel, /YouTube access lacks custom-thumbnail permission — reconnect this channel\./);
assert.match(panel, /needsYoutubeReconnect/);
assert.match(panel, /Reconnect YouTube/);
...
assert.match(panel, /canQueueCandidates/);
assert.match(panel, /Current Golden Nano Banana Pro module snapshotted/);
assert.match(panel, /Render Nano candidate/);
...
assert.match(route, /thumbnailPreviewUrl/);
assert.match(route, /api\/asset-image\?key=/);
...
assert.doesNotMatch(ernieBatchRoute, /requireStudioActor/);
assert.match(ernieBatchRoute, /AUTOMATIC_THUMBNAIL_POLICY_ACTOR_PREFIX/);
assert.match(ernieBatchRoute, /session-free and idempotent/);
...
assert.match(library, /ThumbnailRefreshInventoryPanel[\s\S]*selectedChannelSlug=\{selectedSlug\}[\s\S]*canManage=\{operationsAccess === "owner"\}[\s\S]*canQueueCandidates/);
