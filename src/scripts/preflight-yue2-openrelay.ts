...
async function main() {
  const [organizationId, ceiling] = process.argv.slice(2);
  if (process.argv.length !== 4 || !organizationId || !/^\d+$/u.test(ceiling ?? "")) {
    throw new Error("usage: tsx src/scripts/preflight-yue2-openrelay.ts ORGANIZATION_UUID MAX_HOURLY_CENTS");
  }
  console.log(JSON.stringify(await inspectYuE2OpenRelayAdmission({
    ...
    maximumHourlyCents: Number(ceiling),
  }), null, 2));
}
