...
export const GOLDEN_THUMBNAIL_CRAFT_RULES = [
  ...
  "PUT THE TYPE ON EMPTY GROUND THAT IS STILL PART OF THE WORLD. The reference gives its headline an entire half of SNOW — real environment, receding with the valley, lit by the same sky. It is empty, not blank. NEVER solve this by putting a flat colour panel, gradient block, vignette slab or letterbox band on one side: the scene must extend edge to edge and the quiet area must be a real surface in it — snow, sky, fog, water, a wall, deep shadow. A flat rectangle beside a picture is two images, and it reads as a template.",
