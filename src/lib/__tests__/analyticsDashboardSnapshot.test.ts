...
assert.match(snapshot, /ctx\.db\.query\("youtubeAuth"\)[\s\S]*?withIndex\("by_owner"/);
assert.match(snapshot, /ctx\.db\.query\("analyticsRefreshCursors"\)[\s\S]*?withIndex\("by_owner"/);
