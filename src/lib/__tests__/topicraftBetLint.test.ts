...

{
  ...
  assert.match(source, /resolveTitleProfile\(a\.titleProfile, \{[\s\S]*family: a\.family,[\s\S]*contentLane: a\.contentLane,[\s\S]*niche: a\.niche,[\s\S]*\}\)/, "topicraft must resolve the shared title profile before generation");
